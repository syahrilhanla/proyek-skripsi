...
const BottomProgress = ({ pageProgress }) => {
	const logic = useBottomProgressLogic();
		...
		<div className={progressStyles.main}>
			<div className={progressStyles.content}>
				<span className={progressStyles.left}>
					{logic.currentPath !== 1 && (
						<Link href={`/${logic.prevURL()}`}>
							<ChevronLeftIcon fontSize={"large"} color={"inherit"} onClick={() => useUpdateProgress()} />
						</Link>
					)}
				</span>

				{logic.currentURL.split("/")[2] !== 'kuis' && <span>
				...
				<span className={progressStyles.right}>
					{logic.currentPath < logic.chooseURLList(logic.parentPath)[0].length && (
						<Link href={`/${logic.nextURL()}`} >
							<ChevronRightIcon fontSize={"large"} color={"inherit"} onClick={() => useUpdateProgress()} />
						</Link>
					)}
				</span>
			</div>
